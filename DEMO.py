...
# Function to toggle processing
def toggle_processing(event, x, y, flags, param):
    global processing
    if event == cv2.EVENT_LBUTTONDOWN:
        processing = not processing

# Bind mouse click event to toggle processing
cv2.namedWindow('Emotion Detector')
cv2.setMouseCallback('Emotion Detector', toggle_processing)
...
while True:
    ...
    # If processing flag is False, just show the frame
    if not processing:
